...
def copy_transactions_to_core_tx(**context):
    # 1) Connect to MySQL
    mysql = MySqlHook(mysql_conn_id="mysql_tx_source")

    # 2) Connect to Postgres
    pg = PostgresHook(postgres_conn_id="postgres_core_target")

    # 3) Read all rows from MySQL.transaction
    #    TODO: adjust column list if you don't want SELECT *.
    rows = mysql.get_records("SELECT * FROM transaction")
...
with DAG(
    dag_id="mysql_transaction_to_core_tx",
    start_date=datetime(2024, 1, 1),
    schedule_interval="@daily",  # or None if you only want manual runs
    catchup=False,
    tags=["example", "mysql-to-postgres"],
) as dag:
